...
r3d::light* main_light;

int main()
{
    scene = new r3d::scene(WIDTH, HEIGHT);
    scene->get_camera()->set_position(glm::vec3(0, 0, 1.5f));
    scene->get_camera()->set_rotation(glm::vec3(0, 180, 0));

    main_light = new r3d::light(glm::vec3(0, 8, 1), glm::vec3(0.9, 0.9, 0.9), 45.0f);
    scene->add_light(main_light);

    r3d::shader* diffuse_bump = new r3d::shader(r3d::shader::id::DIFFUSE_TEXTURE_BUMP);
    r3d::material* crate_mat = new r3d::material("assets/crate_diffuse.jpg", "assets/crate_normal.jpg", diffuse_bump);
    r3d::mesh_renderer* crate_renderer = new r3d::mesh_renderer("assets/crate.obj", crate_mat);
    crate = new r3d::game_object("crate", glm::vec3(0, 0, 0), glm::vec3(-90, 0, -90), glm::vec3(1, 1, 1));
